...
%   dts = load(['mha_stats_dts_' num2str(i) '.csv']);
  ma = load(['mha_stats_static' '.csv']);
  ma2 = load(['mha_stats_dynamic' '.csv']);
%   rr = load(['mha_stats_rr_' num2str(i) '.csv']);
  rrt = load(['mha_stats_rrt' '.csv']);
  
  dma_1 = load(['mha_stats_dynamic_1' '.csv']);
  dma_2 = load(['mha_stats_dynamic_2' '.csv']);
  dma_3 = load(['mha_stats_dynamic_3' '.csv']);
  
%   success_dts = [success_dts; dts(:,10)>0];
  success_ma = [success_ma; ma(:,10)>0];
  success_ma2 = [success_ma2; ma2(:,10)>0];
  
  success_dma_1 = [success_dma_1; dma_1(:,10)>0];
  success_dma_2 = [success_dma_2; dma_2(:,10)>0];
  success_dma_3 = [success_dma_3; dma_3(:,10)>0];
...
%   all_times_dts = [all_times_dts; dts(:,1)];
  ...
  all_times_dma_1 = [all_times_dma_1; dma_1(:,1)];
  all_times_dma_2 = [all_times_dma_2; dma_2(:,1)];
  all_times_dma_3 = [all_times_dma_3; dma_3(:,1)];
...
%   all_exp_dts = [all_exp_dts; dts(:,8)];
  ...
  all_exp_dma_1 = [all_exp_dma_1; dma_1(:,8)];
  all_exp_dma_2 = [all_exp_dma_2; dma_2(:,8)];
  all_exp_dma_3 = [all_exp_dma_3; dma_3(:,8)];
...
idx_dma_1 = (all_times_dma_1 >= 0) & success_dma_1;
idx_dma_2 = (all_times_dma_2 >= 0) & success_dma_2;
idx_dma_3 = (all_times_dma_3 >= 0) & success_dma_3;
% pause;
...
dma1_dma2_dma3 = idx_dma_1 & idx_dma_2 & idx_dma_3;
ma_rrt_ma2 = idx_ma & idx_rrt & idx_ma2
...
%------
centres = [10 20 30 40 50 ]
a = hist(all_times_rrt(ma_rrt_ma2),centres);
b = hist(all_times_ma(ma_rrt_ma2),centres);
c = hist(all_times_ma2(ma_rrt_ma2),centres);
...
% fprintf('ma2 average time:  ma2=%f\n', mean(all_times_ma2(dma1_dma2_dma3)));
% fprintf('ma2 average exp:  ma2=%f\n', mean(all_exp_ma2(dma1_dma2_dma3)));
% fprintf('ma2 average path: ma2_base=%f ma2_obj=%f ma2_arms=%f\n\n', ...
%         mean(ma2_paths.base(dma1_dma2_dma3)), mean(ma2_paths.obj(dma1_dma2_dma3)), mean(ma2_paths.arm_sqr(dma1_dma2_dma3)));