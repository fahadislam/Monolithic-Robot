...
void addRandomObstacles(pcl::PointCloud<pcl::PointXYZ>::Ptr pclCloud, int
    ...
    double surfaceBoundsXMin = 0.5;
    double surfaceBoundsXMax = 11.0;

    double surfaceBoundsYMin = 0.5;
    double surfaceBoundsYMax = 12.5;
    ...
    for (int i = 0, j = 0; i < numSurfaces; ++i, j+=2){
        ...
        // Cuboid(pclCloud, X, Y, Z, dimX, dimY, dimZ, false);
...
vector<Eigen::Vector3d> getVoxelsFromFile(std::string filename){
    ...
    // Doorway
    //  addCuboid(pclCloud, 5, 0, 0, 0.05, 2, 3.0, true);
    ...
    addRandomObstacles(pclCloud, 10, 0);
