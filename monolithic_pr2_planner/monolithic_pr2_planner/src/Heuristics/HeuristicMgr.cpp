...
void HeuristicMgr::update2DHeuristicMaps(const std::vector<unsigned char>& data){   //mike
    int dimX, dimY, dimZ;
    m_occupancy_grid->getGridSize(dimX, dimY, dimZ);
...
bool HeuristicMgr::getGoalPose(int g_x, int g_y, RobotPosePtr& final_pose){
    // RobotPosePtr final_pose;
    bool ik_success = checkIKAtPose(g_x, g_y, final_pose);
    bool collision_free = false;
    if(ik_success)
    ...
    if(collision_free)
    {
        // robot_pose.base_state(final_pose->base_state());
        // robot_pose.right_arm(final_pose->right_arm());
        // robot_pose.left_arm(final_pose->left_arm());
        return true;
    }
    else {
        return false;
...
bool HeuristicMgr::getGoalSample(RobotPosePtr &sample)
{
    printf("sizeeeeee %d\n", validSamples.size());
    if (validSamples.size() == 0)
        return false;
    int ind;
    ind = rand() % validSamples.size();
    sample = validSamples[ind];
}
void HeuristicMgr::initializeMHAHeuristics(const int cost_multiplier){
    ...
    for (size_t i = 0; i < circle_x.size(); ++i) {
        RobotPosePtr goalSample;
        if (getGoalPose(circle_x[i], circle_y[i], goalSample))
        {
            // RobotState statev;
            // statev.base_state(goalSample->base_state());
            // statev.right_arm(goalSample->right_arm());
            // statev.left_arm(goalSample->left_arm());
            // statev.visualize();
            // getchar();
            validSamples.push_back(goalSample);
        }
